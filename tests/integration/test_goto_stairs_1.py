"""Integration test for the stairs-climbing test scenario
(goto_stairs_1.litematic) -- a 3-block climb up a staircase, proving the
bot's real pathfinding climbs by stepping (not jumping) and never falls
off either side of the staircase, against a real client/world. Runs
...
test_goto_jump_1.py -- runs the exact registered "goto_stairs_1" TestCase
...
@pytest.mark.asyncio
async def test_goto_stairs_1_integration(ctx: TestContext):
    registry = TestRegistry()
    ingame_tests.register_default_tests(registry)
    test_case = registry.get("goto_stairs_1")
